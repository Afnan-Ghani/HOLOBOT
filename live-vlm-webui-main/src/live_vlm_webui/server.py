...
def get_serial():
    ...
    if _serial_conn is None or not _serial_conn.is_open:
        try:
            import time; _serial_conn = serial.Serial("/dev/ttyACM0", 9600, timeout=1); time.sleep(2)
        except Exception as e:
            _serial_conn = None
    return _serial_conn

async def pantilt(request):
    global _last_pan, _last_tilt, _serial_conn
    try:
        ...
        if abs(pan_angle - _last_pan) > 2 or abs(tilt_angle - _last_tilt) > 2:
            ser = get_serial()
            if ser:
                cmd = "P{}T{}\n".format(pan_angle, tilt_angle)
...
async def pantilt_options(request):
...
async def create_app(test_mode=False):
    ...
    try:
        global _serial_conn
        import time
        _serial_conn = serial.Serial("/dev/ttyACM0", 9600, timeout=1)
        time.sleep(2)
    ...
    app.router.add_post("/pantilt", pantilt)
    app.router.add_route("OPTIONS", "/pantilt", pantilt_options)
